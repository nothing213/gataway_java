...
//                String temple_total_memory  = totalMemorySize.substring(0,totalMemorySize.length()-1);
                ...
                Float result = (float) (Cpu * 0.5+Io*0.25+memory*0.25);
                System.out.println("最终负载值为："+ result);

                ServerBean serverBean = new ServerBean();
                serverBean.setServerName("ApiManager");
                serverBean.setServerPort("8001");
                serverBean.setServerHost("192.168.2.5");
                serverBean.setServerLB(result);
                serverService.setServerMsg(serverBean);
